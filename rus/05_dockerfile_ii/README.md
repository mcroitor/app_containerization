...
Данная команда устарела и не рекомендуется к использованию. Вместо команды `MAINTAINER` рекомендуется использовать команду `LABEL` для определения метаданных образа.
